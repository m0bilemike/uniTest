...
export default function HomeScreen() {
  ...
  const likedImages = useSelector((state: RootState) => state.images.ids.map(id => state.images.entities[id]).filter(Boolean));

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading, isError, error, refetch } = usePicsumImages();
  ...
  if (isError) return <Text style={{ flex: 1, textAlign: "center", color: textColor }}>Error: {error?.message}</Text>;
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <FlatList
        ...
        refreshing={false}
        onRefresh={refetch}
        ListFooterComponent={isFetchingNextPage ? <Text style={{ textAlign: "center", margin: 10, color: textColor }}>Loading more...</Text> : null}
      />
    </View>
  );
}
